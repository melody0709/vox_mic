...
> 状态：准备流程已完成，关键环节已实机验证，可开工 · 2026-09-17
> 标注规则：`[已核实]` = 读过源码/文档确认或实机实测；`[推断]` = 待实测验证
> 已完成：备份（`vox_mic.bak`）、Slint skill 与 CLI 工具、**Slint C++ SDK 1.17.1 安装 + 编译运行实测**、
> 许可档位选定、架构守卫落地
> **开工前必读**：B-fix 依赖 C++20，须排在 B2 之后；B5 并入 B3（见 §9）
...
→ **B-fix 必须排在 B2（17 → 23）之后**，否则编译不过。
若坚持 B-fix 先行，该半条只能写成 C++17 可用形态（放弃 `join()` → `detach()` + 标记残留），
代价是"退不出"时泄漏一个线程、仍不可强杀。**建议先 B2、再 B-fix。**
